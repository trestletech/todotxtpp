...
var endpoint = new DropboxEndpoint(PersistService.User, PersistService.List);
...
.then(function(sessionsKey){
  ...
  app.use(express.session({ 
    cookie  : { maxAge  : 1000 * 60 * 60 * 24 * 7 }, //TODO: I think this always defaults to 24 hours regardless of what we put here.
    secret: sessionsKey.toString('base64'),
    store: store,
    rolling: true // reset cookie expiration
  }));
  ...
  app.get('/', function(req, res, next){
    ...
    if (req.user && req.user.filters){
      vars.filters = req.user.filters;
    }
  ...
  app.use('/filters', function(req, res){
    var filters = req.user.filters || [];
    var filter = req.body.filter || req.query.filter;
    var action = req.body.action || req.query.action || 'add';
    ...
    if (action === 'add'){
      ...
      req.user.filters = filter;
    } else{
      return res.end(400, "Unsupported operation.");
    }
